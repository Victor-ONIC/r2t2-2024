/***********************************************************************
 * Motion_Control_and_Grippers_SerialV3.5.ino
...
#define tofGX 18 // INT3 D5 sur UNO
#define tofGY 2  // INT4. D2 sur UNO
#define tofGZ 3  // INT5. D3 sur UNO

/* From  Axis TOF Pins(Gestion par PortChange interrupt */
#define tofXY 12  // PCINT6 D6 sur UNO
#define tofYZ 13  // PCINT7. D7 sur UNO
#define tofZX 14   // PCINT10. D8 sur UNO
...
bool _end=false;
int tofG[6]={tofGX, tofGY, tofGZ,tofXY, tofYZ,tofZX};

//ISR(PCINT0_vect)
//{
// if(digitalRead(tofXY)==0 ||digitalRead(tofYZ)==0 ) {
//    Serial.println("tof Axe XY ou YZ  : Low");
//    mainOrder = 'T';
//  }
//  if((digitalRead(tofXY)==1) ||(digitalRead(tofYZ)==1 )){
//    Serial.println("tof Axe Xy ou YZ : High");
//    mainOrder = 'S';
//  } 
//}
//
//ISR(PCINT1_vect)
//{
//if(digitalRead(tofZX)==0 ) {
//    Serial.println("tof Axe ZX : Low");
//    mainOrder = 'T';
//  }
//  if(digitalRead(tofZX)==1){
//    Serial.println("tof Axe ZX : High");
//    mainOrder = 'S';
//  }   
//}
...
// For Axis X,Y, Z
  ...
  attachInterrupt(digitalPinToInterrupt(tofGX), tofGXChange, CHANGE);
  attachInterrupt(digitalPinToInterrupt(tofGY), tofGYChange, CHANGE);
  attachInterrupt(digitalPinToInterrupt(tofGZ), tofGZChange, CHANGE);
  
 // Traitement par interruption Pin Change pour tof sur axes
//cli();
//PCICR |=0x030;// Actif sur PORTB ET PORTJ
//DDRB &=~(11<<PORTB6);// PB6 et PB7 en entrée
//DDRJ &=~(1<<PORTJ1); //PJ1 en entrée
//PCMSK0=0xC0;
//PCMSK1=0x04;
//sei();
  delay(100);
...
void tofGXChange(){
  if(digitalRead(tofGX)==0) {
    Serial.println("tof GripperX  : Low");
    mainOrder = 'T';
  }
  if(digitalRead(tofGX)==1){
    Serial.println("tof GripperX : High");
    mainOrder = 'S';
  }
}
void tofGYChange(){
  if(digitalRead(tofGY)==0) {
    Serial.println("tof GripperY : Low");
    mainOrder = 'T';
  }
  if(digitalRead(tofGY)==1){
    Serial.println("tof GripperY : High");
    mainOrder = 'S';
  }
}
void tofGZChange(){
  if(digitalRead(tofGZ)==0) {
    Serial.println("tof GripperZ : Low");
    mainOrder = 'T';
  }
  if(digitalRead(tofGZ)==1){
